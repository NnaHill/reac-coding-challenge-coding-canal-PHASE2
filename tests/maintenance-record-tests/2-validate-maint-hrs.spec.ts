...
test.describe('Maintenance Record Form - Hours Spent Validation', () => {
  ...
  test('should reject negative values for hours spent', async ({ page }) => {
    ...
    // Fill in a negative value for hours spent
...
await page.waitForSelector('.error-message, .text-red-500, [role="alert"]', { timeout: 5000 });
    ...
    // Wait for a short period to allow for client-side validation
    await page.waitForTimeout(1000);
    // Check for any error message
    const errorMessage = await page.evaluate(() => {
      return document.body.innerText;
    });
    console.log('Full page text:', errorMessage);
    if (errorMessage.toLowerCase().includes('hours spent') && errorMessage.toLowerCase().includes('negative')) {
